...
impl LinkedListAllocator {
    ...
    pub unsafe fn init(&mut self, heap_start: usize, heap_size: usize) {
        ...
        super::HEAP_END = heap_start + heap_size;
    }
...
unsafe impl GlobalAlloc for Locked<LinkedListAllocator> {
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        ...
        if let Some((region, alloc_start)) = allocator.find_region(size, align) {
            ...
            let mut number = size / 0x1000;
            if number % 0x1000 > 0 {
                number += 1;
            }
            // TODO check whether the number of allocated frames is enough
            let allocated = syscall::memrequest(number);
            allocator.add_free_region(super::HEAP_END, allocated * 0x1000);
            super::HEAP_END += allocated * 0x1000;
            if let Some((region, alloc_start)) = allocator.find_region(size, align) {
                let alloc_end = alloc_start.checked_add(size).expect("overflow");
                let excess_size = region.end_addr() - alloc_end;
                if excess_size > 0 {
